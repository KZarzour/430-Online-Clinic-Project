...
@app.route('/checkPatient', methods=['POST'])
def checkPatient():
    username = request.json['username']
    password = request.json['password']
    match = patients.query.filter_by(username=username, password = password).first()
    if match:
        return jsonify(match.id,match.fname)
    else:
        return jsonify(False)
        

@app.route('/checkDoctor', methods=['POST'])
def checkDoctor():
    username = request.json['username']
    password = request.json['password']
...
@app.route("/specificAppointment", methods=["POST"])
def appt():
...
def updatePatient():
    ...
    username=request.json['username']
    password = request.json['password']
    match = patients.query.filter_by(id=id).first()
    if match:
        if fname != "":
            match.fname = fname
        if lname != "":
...
def updateDoctor():
    ...
    match = doctors.query.filter_by(id=id).first()
    if match:
        if fname != "":
            match.fname = fname
        if lname != "":
